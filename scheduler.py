...
def build_member_pools(members: list[dict]) -> dict:
    ...
    - Media Coordinator role  → ONLY members with status 'mediaCoordinator'
    - Sound Coordinator role  → ONLY members with status 'soundCoordinator'
    - A coordinator (media or sound) CANNOT be used in any other role
    - Camera roles → members with 'camera' in roles[] (non-coordinators only)
    - Sound roles  → members with 'sound' in roles[] (non-coordinators only)
    """
    # Base eligible pool (no excluded statuses)
    eligible = [
        m for m in members
        if not any(s in EXCLUDED_STATUSES for s in (m.get("statuses") or []))
    ]

    # Coordinator members are locked out of regular roles
    ...
    for role_key, _, _, _, special in ROLE_DEFINITIONS:
        ...
        elif role_key in CAMERA_ROLES:
            pool = [m for m in regular if "camera" in (m.get("roles") or [])]
        elif role_key in SOUND_ROLES:
            pool = [m for m in regular if "sound" in (m.get("roles") or [])]
        elif role_key == "videoMixer":
            pool = [m for m in regular if "videoMixer" in (m.get("roles") or [])]
        elif role_key == "light":
            pool = [m for m in regular if "light" in (m.get("roles") or [])]
        elif role_key == "picture":
            pool = [m for m in regular if "camera" in (m.get("roles") or [])]
        elif role_key == "mobileCamera":
            # Only members who specifically have 'mobileCamera' in their roles[]
            # This enables sticky assignment if only one person is trained
            pool = [m for m in regular if "mobileCamera" in (m.get("roles") or [])]
            # Fallback: if no one has mobileCamera specifically, use camera-trained members
            if not pool:
                pool = [m for m in regular if "camera" in (m.get("roles") or [])]
        else:
            pool = [m for m in regular if role_key in (m.get("roles") or [])]
...
def generate_schedule(year: int, quarter: int, members: list[dict]) -> dict:
    ...
    def pick(role_key: str, already_used_this_sunday: set) -> int | None:
        ...
        if not candidates:
            # Relax global constraint (fallback for small pools)
