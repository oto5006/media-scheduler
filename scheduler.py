...
def build_member_pools(members: list[dict]) -> dict:
    """
    Build per-role eligible member pools, enforcing:
    - Excluded statuses cannot appear anywhere
    - mediaCoordinator role → only status mediaCoordinator members
    - soundCoordinator role → only status soundCoordinator members
    - Camera roles → members with 'camera' in their roles[]
    - Sound roles → members with 'sound' in their roles[]
    - videoMixer, mobileCamera, picture, light → matching role key in roles[]
    """
    # Eligible base pool (no excluded statuses)
    eligible = [
        m for m in members
    ...
    for role_key, _, _, _, special in ROLE_DEFINITIONS:
        if special == "mediaCoordinator":
            ...
            pool = [m for m in eligible if "camera" in (m.get("roles") or [])]
        elif role_key in SOUND_ROLES:
            pool = [m for m in eligible if "sound" in (m.get("roles") or [])]
        elif role_key == "videoMixer":
            pool = [m for m in eligible if "videoMixer" in (m.get("roles") or [])]
        elif role_key == "light":
            pool = [m for m in eligible if "light" in (m.get("roles") or [])]
        elif role_key == "picture":
            pool = [m for m in eligible if "camera" in (m.get("roles") or [])]
        elif role_key == "mobileCamera":
            pool = [m for m in eligible if "camera" in (m.get("roles") or [])]
        else:
            pool = [m for m in eligible if role_key in (m.get("roles") or [])]
...
def generate_schedule(year: int, quarter: int, members: list[dict]) -> dict:
    ...
    def pick(role_key: str, already_used_this_sunday: set) -> int | None:
        """Pick the least-used eligible member for a role, respecting global once-per-schedule rule."""
        candidates = [
            mid for mid in pools[role_key]
            if mid not in used_globally and mid not in already_used_this_sunday
        ]
        if not candidates:
            # Relax global constraint (fallback — shouldn't happen in normal pools)
